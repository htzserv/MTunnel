...
# When the installer itself is copied to a disconnected server, preserve every bundled module
# in the local cache. Only the bootstrap set is deployed immediately; the rest stays lazy.
if [ "$MODE" != "online" ] && [ -d "$SCRIPT_DIR" ] && [ "$SCRIPT_DIR" != "$LOCAL_DIR" ]; then
    for bundled in "$SCRIPT_DIR"/*.sh; do
        [ -s "$bundled" ] || continue
        name="$(basename "$bundled")"
        [ -s "$LOCAL_DIR/$name" ] || cp -f "$bundled" "$LOCAL_DIR/$name"
...
download_to_cache(){
    local f="$1" tmp="$LOCAL_DIR/.${f}.$$"
    rm -f "$tmp"
    if command -v curl >/dev/null 2>&1; then
        curl -fL --retry 2 --connect-timeout 8 --max-time 120 --progress-bar \
            -o "$tmp" "$REPO_SCRIPTS/$f?v=$(date +%s)" || { rm -f "$tmp"; return 1; }
    else
        wget --timeout=8 --tries=2 -O "$tmp" "$REPO_SCRIPTS/$f?v=$(date +%s)" || { rm -f "$tmp"; return 1; }
    fi
...
failed=()
for i in "${!BOOTSTRAP_MODULES[@]}";do f="${BOOTSTRAP_MODULES[$i]}"; label="${f%.sh}";[ "$f" = mstats.sh ]&&label=mstat; src="";[ "$MODE" != online ]&&src="$(find_local "$f"||true)"; if [ -n "$src" ];then echo -e "  ${G}✓${NC} $label ${DIM}(local)${NC}";deploy "$f" "$src"||failed+=("$label");elif [ "$MODE" = offline ];then echo -e "  ${R}✗${NC} $label ${R}(not available locally)${NC}";failed+=("$label");else
    echo -e "  ${C}→${NC} $label ${DIM}(GitHub → local cache)${NC}"
    if download_to_cache "$f" && deploy "$f" "$LOCAL_DIR/$f"; then
        :
    else
        failed+=("$label")
    fi
fi
progress "$((i+1))" "${#BOOTSTRAP_MODULES[@]}" "$label"
done
