#!/bin/bash
# --- MHDesign Master Core | Central Dashboard v1.2 ---
...
while true; do
    ...
    case $opt in
        1) 
           ...
           else echo -e "\n  ${R}● Error: mgre.sh module not found!${NC}"; sleep 2; fi ;;
           
        2) 
           if [ -f "$MPORTER_MODULE" ] && [ -x "$MPORTER_MODULE" ]; then $MPORTER_MODULE;
           elif [ -f "./mporter.sh" ]; then bash ./mporter.sh;
           else echo -e "\n  ${R}● Error: mporter.sh module not found!${NC Pap}"; sleep 2; fi ;;
        ...
        4) 
           ...
           if [[ "$del_confirm" == "y" ]]; then
               ...
               # حذف تمام تانل‌های ثبت شده و قوانین سورس سیستم مالتی تانل
               ...
               # ردیابی و پاکسازی هرگونه اینترفیس باقی‌مانده مربوط به ما
               ...
               # حذف پوشه جاری که اسکریپت‌ها در آن در حال اجرا هستند
               current_dir=$(pwd)
