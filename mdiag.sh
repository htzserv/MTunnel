...
run_full_infrastructure_scan() {
    ...
    for conf in "$GRE_DIR"/*.conf "$VX_DIR"/*.conf "$L2TP_DIR"/*.conf "$HYS_DIR"/*.conf; do
        [ ! -f "$conf" ] && continue; has_any=true; unset TYPE LOCAL_PUB REMOTE_PUB MAX_IPS SYNC_KEY TUN_SECRET T_NAME TUN_ID CORE_SUBNET TUN_PROTO LOCAL_IP6 REMOTE_IP6 VNI_ID BR_NAME TUN_PORT HYS_PASS VX_NAME 2>/dev/null; source "$conf"
