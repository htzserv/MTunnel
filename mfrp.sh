...
deploy_frp_server() {
    ...
    auth_token=$(echo "$auth_token" | tr -d '\r' | tr -d ' ')
...
view_frp_config() {
    ...
    echo -e "\n  ${C}●${NC} ${W}Press 'q' to exit the logs.${NC}"
    echo -ne "  ${DIM}Press Enter to open logs...${NC}"; read dummy
    
    if systemctl is-active --quiet frps 2>/dev/null; then journalctl -u frps -n 30 --no-pager
    elif systemctl is-active --quiet frpc 2>/dev/null; then journalctl -u frpc -n 30 --no-pager
    else echo -e "  ${R}● No active FRP services to track.${NC}"; fi
...
restart_frp_services() {
    ...
    r_opt=$(echo "$r_opt" | tr -d '\r')
    case $r_opt in
        1) systemctl restart frps 2>/dev/null; echo -e "  ${G}● FRP Server restarted successfully!${NC}"; sleep 1.5 ;;
        2) systemctl restart frpc 2>/dev/null; echo -e "  ${G}● FRP Client restarted successfully!${NC}"; sleep 1.5 ;;
        0) return ;; *) echo -e "  ${R}● Invalid Option!${NC}"; sleep 1 ;;
    esac
}
...
while true; do
    ...
    opt=$(echo "$opt" | tr -d '\r')
    case $opt in
        1) deploy_frp_server ;; 2) deploy_frp_client ;; 3) view_frp_config ;; 4) restart_frp_services ;;
        5) 
           ...
           if [[ "$confirm" == "y" ]]; then
               ...
               systemctl daemon-reload; echo -e "  ${G}● FRP Services completely wiped.${NC}"; sleep 1.5
           fi ;;
        0) break ;;
    esac
done
