        ...
        1) 
           while true; do
               ...
               if [[ -z "$user_tun_id" ]] || [[ ! "$user_tun_id" =~ ^[0-9]+$ ]] || [ "$user_tun_id" -lt 1 ] || [ "$user_tun_id" -gt 250 ]; then
                   ...
                   user_tun_id=$((RANDOM % 200 + 10))
               fi
               
               while grep -q "TUN_ID=$user_tun_id$" "$CONF_DIR"/*.conf 2>/dev/null; do
                   echo -e "  ${R}● Error: ID $user_tun_id is already used by another tunnel on this server!${NC}"
                   echo -ne "  ${C}●${NC} ${W}Enter a different Network ID: ${NC}"; read -t 30 user_tun_id
               done
               tun_id=$user_tun_id
