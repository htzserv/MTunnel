#!/bin/bash
# --- MHysteria Modular Core | Hysteria2+WG Engine v1.1.0 ---
...
while true; do
    ...
    case $opt in
        ...
        3)
           ...
           if [[ "$del_idx" == "all" ]]; then
               for conf in "${configs[@]}"; do TYPE=""; LOCAL_PUB=""; REMOTE_PUB=""; MAX_IPS="0"; SYNC_KEY=""; TUN_SECRET=""; T_NAME=""; TUN_ID=""; CORE_SUBNET=""; TUN_PROTO="ipv4"; LOCAL_IP6=""; REMOTE_IP6=""; VNI_ID=""; BR_NAME=""; TUN_PORT=""; HYS_PASS=""; VX_NAME=""; source "$conf"; systemctl stop mhysteria@$T_NAME 2>/dev/null; systemctl disable mhysteria@$T_NAME 2>/dev/null; ip link del "$T_NAME" 2>/dev/null; rm -rf "$CONF_DIR/$T_NAME" "$conf"; done
               [ -x "/usr/bin/mporter" ] && /usr/bin/mporter --cleanup-orphans >/dev/null 2>&1 &
           else
               TYPE=""; LOCAL_PUB=""; REMOTE_PUB=""; MAX_IPS="0"; SYNC_KEY=""; TUN_SECRET=""; T_NAME=""; TUN_ID=""; CORE_SUBNET=""; TUN_PROTO="ipv4"; LOCAL_IP6=""; REMOTE_IP6=""; VNI_ID=""; BR_NAME=""; TUN_PORT=""; HYS_PASS=""; VX_NAME=""; source "${configs[$del_idx]}"; systemctl stop mhysteria@$T_NAME 2>/dev/null; ip link del "$T_NAME" 2>/dev/null; rm -rf "$CONF_DIR/$T_NAME" "${configs[$del_idx]}"
               [ -x "/usr/bin/mporter" ] && /usr/bin/mporter --cleanup-orphans >/dev/null 2>&1 &
