...
while true; do
    ...
    opt=$(echo "$opt" | tr -d '\r')
    case $opt in
        1) activate_firewall ;; 2) disable_firewall ;; 3) deploy_receiver ;; 4) list_obfs ;; 0) exit 0 ;;
    esac
done
