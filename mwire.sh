#!/bin/bash
# --- MWIRE Crypto Secure Matrix (mwire.sh) | MDesign Core v1.4.0 (Sanitized) ---
...
draw_mwire_header() {
    ...
    if pgrep -x "wg-crypt-wg0" >/dev/null 2>&1 || ip link show wg0 >/dev/null 2>&1; then
        srv_status="ONLINE"; srv_color="${G}"; active_peers=$(wg show wg0 peers 2>/dev/null | wc -l)
    fi
    clear; echo ""
    local str1=" MWIRE Crypto Matrix 1.4.0 "
    local raw_len=$(( ${#str1} + 1 + 5 + ${#s_ip} + 1 + 10 + ${#srv_status} + 1 + 9 + ${#active_peers} ))
    local pad_len=$(( 92 - raw_len )); if (( pad_len < 0 )); then pad_len=0; fi; local padding=$(printf '%*s' "$pad_len" "")
    echo -e "  ${B}╭────────────────────────────────────────────────────────────────────────────────────────────╮${NC}"
...
init_wg_server() {
    mkdir -p "$WG_DIR" "$PEER_DIR" 2>/dev/null
    chmod 700 "$WG_DIR" "$PEER_DIR" 2>/dev/null

    if [ -f "$WG_CONF" ]; then echo -e "\n  ${Y}● WireGuard Server is already initialized!${NC}"; sleep 1.5; return; fi
    echo -e "\n  ${DIM}┌─[ INITIALIZE CRYPTO SERVER Core ]${NC}"
    
    local wg_port="51820"
    while true; do
        echo -ne "  ${C}●${NC} ${W}WireGuard Listen Port [${Y}51820${W}] (Or 'q' to Cancel): ${NC}"; read input_port
        input_port=$(echo "$input_port" | tr -d '\r' | tr -d ' ')
        [[ "$input_port" == "q" ]] && return
    ...
    while true; do
        ...
        input_sub=$(echo "$input_sub" | tr -d '\r' | tr -d ' ')
        [[ "$input_sub" == "q" ]] && return
        [ -n "$input_sub" ] && wg_subnet=$input_sub
        break
    done

    local server_priv=$(wg genkey); local server_pub=$(echo "$server_priv" | wg pubkey)
...
EOF
    chmod 600 "$WG_CONF"
    systemctl enable wg-quick@wg0 >/dev/null 2>&1; systemctl start wg-quick@wg0 >/dev/null 2>&1

    echo -e "\n  ${G}● WireGuard Server deployed successfully!${NC}"
    echo -e "  ${DIM}├─ Public Key: ${W}${server_pub}${NC}\n  ${DIM}└─ Listen Port: ${Y}${wg_port}${NC}"; sleep 3
}

generate_peer() {
    mkdir -p "$WG_DIR" "$PEER_DIR" 2>/dev/null
    if [ ! -f "$WG_CONF" ]; then echo -e "\n  ${R}● Initialize WireGuard server core first (Option 1)!${NC}"; sleep 2; return; fi

    echo -e "\n  ${DIM}┌─[ GENERATE CRYPTO PEER MATRIX ]${NC}"
    local p_name=""
    while true; do
        echo -ne "  ${C}●${NC} ${W}Enter Peer Name (No spaces | q:Back): ${NC}"; read p_name
        p_name=$(echo "$p_name" | tr -d '\r' | tr -d ' ')
        [[ "$p_name" == "q" ]] && return
        [[ -z "$p_name" ]] && continue
    ...
    local base_ip=$(echo "$server_ip" | cut -d'.' -f1-3); local last_octet=$(echo "$server_ip" | cut -d'.' -f4)
    ...
    local peer_priv=$(wg genkey); local peer_pub=$(echo "$peer_priv" | wg pubkey); local peer_psk=$(wg genpsk)
...
EOF
    ...
    echo -e "\n  ${G}● Peer Matrix generated successfully!${NC}\n  ${DIM}├─ Peer File: ${W}${PEER_DIR}/${p_name}.conf${NC}\n  ${DIM}└─ Allocated IP: ${Y}${peer_ip}${NC}\n"
    echo -e "  ${C}▼ CLIENT CONFIGURATION TEXT:${NC}\n${DIM}────────────────────────────────────────────────────────────────────────────────────────────${NC}"
    cat "$PEER_DIR/${p_name}.conf"
    echo -e "${DIM}────────────────────────────────────────────────────────────────────────────────────────────${NC}"
    
    if command -v qrencode >/dev/null 2>&1; then echo -e "\n  ${M}▼ MOBILE SCAN SCANNER (QR CODE):${NC}"; qrencode -t ansiutf8 < "$PEER_DIR/${p_name}.conf"; fi
    echo -ne "\n  ${DIM}Press Enter to return...${NC}"; read dummy
}

show_active_peers() {
    if [ ! -f "$WG_CONF" ] || ! ip link show wg0 >/dev/null 2>&1; then echo -e "\n  ${R}● WireGuard interface wg0 is inactive or not deployed!${NC}"; sleep 2; return; fi
    echo -e "\n  ${C}Live Encryption Engine Tunnel Matrix:${NC}\n  ${B}╭────────────────────────────────────────────────────────────────────────────────────────────╮${NC}"
    while read -r line; do printf "  ${B}│${NC}  %-88s  ${B}│${NC}\n" "$line"; done < <(wg show wg0)
    echo -e "  ${B}╰────────────────────────────────────────────────────────────────────────────────────────────╯${NC}"
    echo -ne "  ${DIM}Press Enter to return...${NC}"; read dummy
}

remove_peer_or_server() {
    if [ ! -f "$WG_CONF" ]; then echo -e "\n  ${R}● No active WireGuard deployment detected!${NC}"; sleep 1.5; return; fi
    echo -e "\n  ${DIM}┌─[ DESTROY MATRIX COMPONENTS ]${NC}\n  ${DIM}├─${NC} ${W}1${NC} ${DIM}❯${NC} ${Y}Remove Specific Client Peer${NC}\n  ${DIM}├─${NC} ${W}2${NC} ${DIM}❯${NC} ${R}Completely Wipe WireGuard Core Server${NC}\n  ${DIM}└─${NC} ${W}q${NC} ${DIM}❯${NC} ${DIM}Cancel${NC}"
    
    local choice=""
    while true; do echo -ne "  ${C}●${NC} ${W}Select Destruction Target: ${NC}"; read choice; choice=$(echo "$choice" | tr -d '\r'); [[ "$choice" == "q" ]] && return; [[ "$choice" == "1" || "$choice" == "2" ]] && break; done
    ...
    if [[ "$choice" == "1" ]]; then
        ...
        for i in "${!peer_files[@]}"; do printf "  ${B}│${NC}  ${Y}%-3s${NC} ${C}❯${NC} ${W}%-53s${NC} ${B}│${NC}\n" "$i" "$(basename "${peer_files[$i]}" .conf)"; done
        echo -e "  ${B}├────────────────────────────────────────────────────────────┤${NC}"
        printf "  ${B}│${NC}  ${Y}%-3s${NC} ${C}❯${NC} ${DIM}%-53s${NC} ${B}│${NC}\n" "q" "Cancel and Return"
        echo -e "  ${B}╰────────────────────────────────────────────────────────────╯${NC}"
        
        local p_idx=""
        while true; do echo -ne "  ${C}●${NC} ${W}Select Index: ${NC}"; read p_idx; p_idx=$(echo "$p_idx" | tr -d '\r'); [[ "$p_idx" == "q" ]] && return; [[ -n "$p_idx" && -n "${peer_files[$p_idx]}" ]] && break; done
        ...
        echo -ne "  ${R}● DANGER: Completely wipe WireGuard server and all clients? (y/n): ${NC}"; read confirm; confirm=$(echo "$confirm" | tr -d '\r')
        if [[ "$confirm" == "y" ]]; then
            systemctl stop wg-quick@wg0 >/dev/null 2>&1; systemctl disable wg-quick@wg0 >/dev/null 2>&1; rm -rf "$WG_DIR"
...
import_client_config() {
    mkdir -p "$WG_DIR" 2>/dev/null
    if [ -f "$WG_CONF" ]; then echo -e "\n  ${R}● WireGuard configuration already exists on this server!${NC}\n  ${DIM}├─ Delete the old one first (Option 4).${NC}"; sleep 2.5; return; fi

    echo -e "\n  ${DIM}┌─[ IMPORT CLIENT CONFIGURATION ]${NC}\n  ${DIM}│${NC} ${Y}Paste the entire configuration text generated by your Server here.${NC}\n  ${DIM}│${NC} ${Y}When you are finished pasting, type ${W}SAVE${Y} on a new line and press Enter.${NC}\n  ${DIM}└────────────────────────────────────────────────────────────────────────${NC}\n"

    local temp_conf="/tmp/wg0_import.conf"; > "$temp_conf"
    while IFS= read -r line; do line=$(echo "$line" | tr -d '\r'); [[ "$line" == "SAVE" || "$line" == "save" ]] && break; echo "$line" >> "$temp_conf"; done

    if [ ! -s "$temp_conf" ]; then echo -e "\n  ${R}● Error: Empty configuration received! Import aborted.${NC}"; rm -f "$temp_conf"; sleep 2; return; fi

    mv "$temp_conf" "$WG_CONF"; chmod 600 "$WG_CONF"
    systemctl enable wg-quick@wg0 >/dev/null 2>&1; systemctl start wg-quick@wg0 >/dev/null 2>&1

    if ip link show wg0 >/dev/null 2>&1; then echo -e "\n  ${G}● Client Node deployed successfully! Connected to Matrix.${NC}\n  ${DIM}├─ Interface: ${W}wg0${NC}\n  ${DIM}└─ Status: ${G}ONLINE${NC}"
    else echo -e "\n  ${R}● Failed to start WireGuard interface. Check configuration syntax.${NC}"; fi
    sleep 3
}
...
while true; do
    ...
    echo -ne "  ${G}MWIRE ❯❯ ${NC}"; read opt; opt=$(echo "$opt" | tr -d '\r')
    case $opt in
        1) init_wg_server ;; 2) generate_peer ;; 3) show_active_peers ;; 4) remove_peer_or_server ;; 5) import_client_config ;; 0) break ;;
    esac
done
