#!/bin/bash
# --- MXLAN Layer-2 Fabric (mxlan.sh) | MDesign Core v1.2.1 (Auto-Ping & MPorter Integration) ---
        ...
        4)
           ...
           if [[ "$del_idx" == "all" ]]; then
               ...
               if [[ "$confirm_all" == "y" ]]; then
                   for conf in "${configs[@]}"; do TYPE=""; LOCAL_PUB=""; REMOTE_PUB=""; MAX_IPS="0"; SYNC_KEY=""; TUN_SECRET=""; T_NAME=""; TUN_ID=""; CORE_SUBNET=""; TUN_PROTO="ipv4"; LOCAL_IP6=""; REMOTE_IP6=""; VNI_ID=""; BR_NAME=""; TUN_PORT=""; HYS_PASS=""; VX_NAME=""; source "$conf"; ip link del "$VX_NAME" >/dev/null 2>&1; ip link del "$BR_NAME" >/dev/null 2>&1; rm -f "$conf" "${STATE_DIR}/${VX_NAME}.state"; done
           ...
           if [[ -n "${configs[$del_idx]}" ]]; then
               TYPE=""; LOCAL_PUB=""; REMOTE_PUB=""; MAX_IPS="0"; SYNC_KEY=""; TUN_SECRET=""; T_NAME=""; TUN_ID=""; CORE_SUBNET=""; TUN_PROTO="ipv4"; LOCAL_IP6=""; REMOTE_IP6=""; VNI_ID=""; BR_NAME=""; TUN_PORT=""; HYS_PASS=""; VX_NAME=""; source "${configs[$del_idx]}"; ip link del "$VX_NAME" >/dev/null 2>&1; ip link del "$BR_NAME" >/dev/null 2>&1
