...
show_fabric_details() {
    ...
    if [ ${#configs[@]} -eq 0 ]; then echo -e "\n  ${R}● No VXLAN fabrics configured yet!${NC}"; sleep 1.5; return; fi

    echo -e "\n  ${M}● Deployed Fabrics Registry (Scanning Network Latency...):${NC}"
    for conf in "${configs[@]}"; do
        ...
        local t_sec="[ N/A for VXLAN ]"
        
        echo -e "  ${B}╭────────────────────────────────────────────────────────────────────────────────────────────╮${NC}"
        local left_p="▼ Fabric: $VX_NAME"
        ...
        local l2="Fabric Secret: ${t_sec}"; local r2="Network VNI: ${VNI_ID}"
        local pad2=$(( 89 - ${#l2} - ${#r2} )); [ "$pad2" -lt 0 ] && pad2=0; local sp2=$(printf '%*s' "$pad2" "")
        echo -e "  ${B}│${NC} ${C}Fabric Secret:${NC} ${DIM}${t_sec}${NC}${sp2} ${DIM}Network VNI:${NC} ${W}${VNI_ID}${NC} ${B}│${NC}"
        ...
        if [[ "$MAX_IPS" -gt 0 ]]; then
            for ((idx=0; idx<MAX_IPS; idx++)); do
                ...
                if [ "$idx" -eq 0 ]; then echo -e "  ${B}│${NC} ${DIM}Virtual IPs  :${NC} ${M}${v_lip}${NC} ${DIM}->${NC} ${M}${v_tip}${NC}${p_sp} ${v_stat_color}${r_str}${NC} ${B}│${NC}"
                else echo -e "  ${B}│${NC} ${DIM}               ${M}${v_lip}${NC} ${DIM}->${NC} ${M}${v_tip}${NC}${p_sp} ${v_stat_color}${r_str}${NC} ${B}│${NC}"; fi
            done
        else
...
while true; do
    ...
    case $opt in
        1) 
           ...
           while true; do
               ...
               if [ -f "$CONF_DIR/${vx_name}.conf" ] || ip link show "$vx_name" >/dev/null 2>&1; then
                   echo -e "  ${R}● Error: Fabric interface [${W}${vx_name}${R}] already exists!${NC}"
                   continue
               fi
               break
           done
           ...
           while true; do
               ...
               if ! [[ "$vni_id" =~ ^[0-9]+$ ]] || [ "$vni_id" -lt 1 ] || [ "$vni_id" -gt 16777215 ]; then
                   echo -e "  ${Y}● Invalid VNI. Must be a number between 1 and 16777215.${NC}"
                   continue
               fi
               if grep -q "VNI_ID=$vni_id$" "$CONF_DIR"/*.conf 2>/dev/null; then
                   echo -e "  ${R}● Error: VNI [${W}${vni_id}${R}] is already assigned!${NC}"
                   continue
               fi
               break
           done
           ...
           if ip link show "$vx_name" >/dev/null 2>&1; then
               ...
               echo -e "\n  ${R}● FATAL ERROR: Kernel rejected VXLAN creation!${NC}"
               rm -f "$conf_path"; sleep 3.5
           fi
           ;;
        2)
           ...
           if [[ -n "${configs[$t_idx]}" ]]; then
               ...
               if [[ "$vip_action" == "1" ]]; then
                   ...
                   if [[ "$MAX_IPS" == "0" || -z "$MAX_IPS" ]]; then
                       echo -e "  ${Y}● No Virtual IPs found on this fabric!${NC}"
                       sleep 1.5; continue
                   fi
                   
                   echo -ne "  ${R}● Are you sure you want to delete all ${MAX_IPS} vIPs from [${VX_NAME}]? (y/n): ${NC}"; read confirm_vip
                   if [[ "$confirm_vip" == "y" ]]; then
                       sed -i "s/^MAX_IPS=.*/MAX_IPS=0/" "$sel_conf"
                       sed -i "s/^SYNC_KEY=.*/SYNC_KEY=/" "$sel_conf"
                       apply_fabric "$sel_conf"
                       echo -e "  ${G}● Virtual IPs purged successfully. Core bridge remains intact.${NC}"; sleep 1.5
                   fi
               fi
           fi ;;
        3) 
           while true; do 
               draw_mxlan_header; show_mxlan_monitor
        ...
        4)
           ...
           if [[ "$del_idx" == "all" ]]; then
               echo -ne "  ${R}● DANGER: Are you sure you want to delete ALL VXLAN fabrics? (y/n): ${NC}"; read confirm_all
               if [[ "$confirm_all" == "y" ]]; then
                   ...
                   echo -e "  ${G}● All fabrics have been safely purged.${NC}"; sleep 1.5
               fi
               continue
           fi
