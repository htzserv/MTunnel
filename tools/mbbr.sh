...
echo -e "\n  ${DIM}┌─[ BBR MANAGEMENT ]${NC}\n  ${DIM}├─${NC} ${W}1${NC} ${DIM}❯${NC} ${G}Enable BBR Network Acceleration${NC}\n  ${DIM}├─${NC} ${W}2${NC} ${DIM}❯${NC} ${R}Disable BBR & Remove Settings${NC}\n  ${DIM}└─${NC} ${W}0${NC} ${DIM}❯${NC} ${DIM}Exit${NC}\n"

echo -ne "  ${C}Select ❯❯ ${NC}"; read b_opt
b_opt=$(echo "$b_opt" | tr -d '\r' | tr -d ' ')

case $b_opt in
    1) enable_bbr ;; 2) disable_bbr ;; 0) exit 0 ;; *) echo -e "  ${R}● Invalid option!${NC}" ;;
esac
sleep 1.5
