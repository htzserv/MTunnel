...
import http.server, socketserver, subprocess, urllib.parse, sys, os, json, hashlib, random, re
...
<body class="light-mode">
    ...
    <div id="app-core">
        <div class="sidebar">
            ...
            <div class="side-btn btn-neon-green" id="btn-lang" onclick="toggleLang()" title="Switch Language"></div>
    ...
    <script>
        const i18n = {
            en: {
                ...
                mod_title: "Forward New Port", mod_port: "Local Port", mod_iface: "Target Core Interface", mod_btn: "Deploy Mapping", login_btn: "Secure Login", out: "Logged out successfully.", manual_ip: "Manual IP Entry",
            ...
            fa: {
                ...
                mod_title: "فوروارد پورت جدید", mod_port: "پورت مبدا", mod_iface: "اینترفیس هدف (تونل مپینگ)", mod_btn: "اعمال تنظیمات", login_btn: "ورود ایمن", out: "خروج با موفقیت انجام شد.", manual_ip: "ورود دستی آی‌پی",
        ...
        function openModal() { 
            ...
            latestTunnels.forEach(tObj => {
                if(tObj.type.includes("GRE") || tObj.type.includes("VXLAN")) {
        ...
        function submitAction() {
            ...
            apiPost('add_port', {port: p, dst_ip: i, iface: iface, engine: 'haproxy'});
            closeModal();
...
while true; do
    ...
    for conf in /etc/mgre/tunnels/*.conf /etc/mgre/vxlan/*.conf; do
        if [ -f "$conf" ]; then
            unset TYPE LOCAL_PUB REMOTE_PUB MAX_IPS SYNC_KEY TUN_SECRET T_NAME TUN_ID CORE_SUBNET TUN_PROTO LOCAL_IP6 REMOTE_IP6 VNI_ID BR_NAME VX_NAME
            source "$conf"
            ...
            rip=$(get_tunnel_ip "$name" 2>/dev/null); [ -z "$rip" ] && rip=${REMOTE_PUB:-"Unknown"}
