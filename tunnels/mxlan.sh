#!/bin/bash
# --- MXLAN Layer-2 Fabric (mxlan.sh) | MDesign Core v1.5.0 ---
# [Features: Dynamic Subnet Calculation | L4 Load Balancing | Native NAT Forwarding | MTU Optimized]
...
draw_mxlan_header() {
    ...
    clear; echo ""
    local str1=" MXLAN Layer-2 Edge 1.5.0 "
    local str2=" IP: $s_ip "
...
show_mxlan_monitor() {
    ...
    for conf in "$CONF_DIR"/*.conf; do
        ...
        if [ "$TYPE" == "1" ] && { [ -n "$FWD_TCP" ] || [ -n "$FWD_UDP" ]; }; then
            ...
            fwd_lbl=" ${DIM}| NAT FWD: ${Y}T:[${FWD_TCP:-0}] ${C}U:[${FWD_UDP:-0}] ${DIM}| LB: [${lb_stat}${DIM}]${NC}"
        fi

        echo -e "  ${B}╭────────────────────────────────────────────────────────────────────────────────────────────╮${NC}"
        printf "  ${B}│${NC} %b▼ Fabric: %-35s%b %b\n" "${M}" "${VX_NAME} / ${BR_NAME}" "${NC}" "${fwd_lbl}"
        echo -e "  ${B}├────────────────────┬────────────────────┬────────────────────┬──────────────┬──────────────┤${NC}"
        ...
        1) 
           ...
           if ip link show "$vx_name" >/dev/null 2>&1; then
               ...
               if [[ "$setup_vip" == "y" || "$setup_vip" == "yes" ]]; then
                   ...
                   if [[ "$n" != "q" ]]; then
                       while true; do echo -ne "  ${C}●${NC} ${W}Sync Key: ${NC}"; read k; [[ "$k" == "q" ]] && break; [[ -n "$k" ]] && break; done
                       if [[ "$k" != "q" ]]; then
        ...
        2)
           ...
           if [[ -n "${configs[$t_idx]}" ]]; then
               ...
               if [[ "$vip_action" == "1" ]]; then
                   ...
                   [[ "$n" == "q" ]] && continue
                   while true; do echo -ne "  ${C}●${NC} ${W}Sync Key: ${NC}"; read k; [[ -n "$k" ]] && break; done
                   [[ "$k" == "q" ]] && continue
